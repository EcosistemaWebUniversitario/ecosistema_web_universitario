...
const FormNotas = () => {
  ...
  useEffect(() => {
    const fetchAsignaturas = async () => {
      try {
        ...
        const response = await api.get(
          `/auth/asignaturas/brigada/brigada/${params.idBrigada}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        ...
        await api.put(`/auth/notas/${params.idNota}`, notaData);
        setSuccessMessage("✓ Nota actualizada exitosamente");
      } else {
        // Crear nueva nota
        try {
          const existe = await api.get(
            `/auth/notas/${idAsignatura}/${params.idEstudiante}`
          );
          ...
          await api.post("/auth/notas", notaData);
  ...
  const fetchNota = async (id) => {
    try {
      setLoading(true);
      const response = await api.get(`/auth/notas/${id}`);
