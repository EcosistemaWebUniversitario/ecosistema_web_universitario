      ...
      try {
        ...
        if (response.data[0].id_brigada) {
          const brigadaResponse = await api.get(
            `/auth/brigadas/${response.data[0].id_brigada}`
          );
  ...
  useEffect(() => {
    const fetchPromedio = async () => {
      try {
        ...
        const response = await api.get(
          `/auth/notas/promedio/promedio/${params.id}`
        );
  ...
  useEffect(() => {
    const fetchNotas = async () => {
      try {
        setLoading(true);
        const response = await api.get(
          `/auth/notas/estudiante/${params.id}`
        );
    ...
    if (selectedAño) {
      const fetchNotasPorAño = async () => {
        try {
          setLoading(true);
          const response = await api.get(
            `/auth/notas/estudiante/${params.id}/${selectedAño}`
          );
          const notasData = response.data;
          setNotas(notasData);
          calcularEstadisticas(notasData);
