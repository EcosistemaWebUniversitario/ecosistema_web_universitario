...
router.post("/login", async (req, res) => {
  const { email, password } = req.body;
  console.log("📧 Login intento:", email); // Agrega esto

  try {
    // Busca el usuario por email
    console.log("🔍 Buscando estudiante con email:", email); // Agrega esto
    ...
    if (rows.length === 0) {
      console.log("❌ Estudiante no encontrado"); // Agrega esto
      return res.status(404).json({ message: "usuario no existe" });
    }

    console.log("✅ Estudiante encontrado:", rows[0].correo); // Agrega esto
    // Verifica la contraseña
    const isMatch = await bcrypt.compare(password, rows[0].contraseña);
    console.log("🔑 Contraseña coincide:", isMatch); // Agrega esto
    ...
    // Genera un token JWT
    const token = jwt.sign({ id: rows[0].id_usuario }, "yilian_yailin", {
      expiresIn: "1h",
    });
    console.log("🎟️ Token generado para usuario:", rows[0].id_usuario); // Agrega esto
...
router.post("/asignaturas", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO asignatura(nombre_asignatura) VALUES($1)",
      [nombre],
    );
...
router.get("/asignaturas/nombre/:nombre", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM asignatura WHERE nombre_asignatura = $1",
      [nombre],
    );
...
router.get("/asignaturas/carrera/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM carrera_asignatura ca JOIN asignatura a ON ca.id_asignatura = a.id_asignatura JOIN carrera c ON ca.id_carrera = c.id_carrera WHERE ca.id_carrera = $1",
      [id],
    );
...
router.get("/asignaturas/:carrera/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM carrera_asignatura ca JOIN asignatura a ON ca.id_asignatura = a.id_asignatura JOIN carrera c ON ca.id_carrera = c.id_carrera WHERE ca.id_carrera = $1 AND ca.id_asignatura = $2",
      [carrera, id],
    );
...
router.delete("/asignaturas/:id/:idAsig", async (req, res) => {
  ...
  try {
    await pool.query(
      "DELETE FROM carrera_asignatura WHERE id_carrera = $1 AND id_asignatura = $2",
      [id, idAsig],
    );
    res.status(204).send();
  } catch (error) {
...
router.get("/asignaturas/brigada/brigada/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT a.id_asignatura, a.nombre_asignatura FROM asignatura a  JOIN carrera_asignatura ca ON a.id_asignatura = ca.id_asignatura JOIN carrera c ON ca.id_carrera = c.id_carrera JOIN brigada b ON c.id_carrera = b.id_carrera WHERE b.id_brigada = $1",
      [id],
    );
...
router.post("/facultades", async (req, res) => {
  ...
  try {
    const verificar = await pool.query(
      "SELECT * FROM facultad WHERE nombre_facultad = $1",
      [nombre],
    );
...
router.get("/facultades/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM facultad WHERE id_facultad = $1",
      [id],
    );
...
router.put("/facultades/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE facultad SET nombre_facultad = $1 WHERE id_facultad = $2 RETURNING *",
      [nombre, id],
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ message: "facultad no encontrada" });
...
router.post("/carreras", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO carrera(id_facultad, nombre_carrera, años) VALUES($1, $2, $3)",
...
router.get("/carreras/nombre/:nombre", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM carrera WHERE nombre_carrera = $1",
      [nombre],
    );
...
router.put("/carreras/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE carrera SET id_facultad = $1, nombre_carrera = $2, años = $3 WHERE id_carrera = $4 RETURNING *",
      [id_facultad, nombre_carrera, años, id],
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ message: "carrera no encontrada" });
    }
...
router.post("/brigadas", async (req, res) => {
  const { id_carrera, nombre_brigada, año_brigada, añoFinal_brigada } =
    req.body;
  try {
    const result = await pool.query(
      "INSERT INTO brigada(id_carrera, nombre_brigada, año_brigada, añoFinal_brigada) VALUES($1, $2, $3, $4)",
...
router.get("/brigadas/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM brigada WHERE id_brigada = $1",
      [id],
    );
...
router.get("/brigadas/carrera/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM brigada WHERE id_carrera = $1",
      [id],
    );
    res.json(result.rows);
  } catch (err) {
...
router.get("/brigadas/nombre/:nombre", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM brigada WHERE nombre_brigada = $1",
      [nombre],
    );
...
router.post("/estudiantes", async (req, res) => {
  const { correo, contraseña, id_brigada, nombre_estudiante, carnet } =
    req.body;
  try {
    const hashPassword = await bcrypt.hash(contraseña, 10);
    const result = await pool.query(
      'INSERT INTO estudiante(correo, contraseña, rol, id_brigada, nombre_estudiante, carnet) VALUES($1, $2, "estudiante", $3, $4, $5)',
      [correo, hashPassword, id_brigada, nombre_estudiante, carnet],
    );
    res.status(201).json(result.rows[0]);
  } catch (error) {
    console.error(error);
...
router.get("/estudiantes/usuario/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM estudiante WHERE id_usuario = $1",
      [id],
    );
...
router.get("/estudiantes/brigada/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM estudiante WHERE e.id_brigada = $1 GROUP BY e.id_estudiante",
      [id],
    );
    res.json(result.rows);
  } catch (err) {
...
router.post("/notas", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO nota(id_estudiante, id_asignatura, valor, año) VALUES($1, $2, $3, $4)",
...
router.get("/notas/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM nota n JOIN asignatura a ON n.id_asignatura = a.id_asignatura WHERE id_nota = $1",
      [id],
    );
...
router.put("/notas/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE nota SET id_estudiante = $1, id_asignatura = $2, valor =$3, año = $4  WHERE id_nota = $5 RETURNING *",
      [id_estudiante, id_asignatura, valor, año, id],
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ message: "nota no encontrada" });
    }
...
router.get("/notas/promedio/promedio/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT ROUND(AVG(valor), 1) FROM nota WHERE id_estudiante = $1",
      [id],
    );
    res.json(result.rows);
